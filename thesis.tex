...
\begin{document}
	...
	\chapter{Methodology}
		...
		\section{Reduced Order Model}
			This section is formulated in accordance with \cite{ROWLEY2004115}, \cite{Pinnau2008}, \cite{brunton_kutz_2019}, \cite{Arunajatesan20144} and \cite{tubiblio70572}. 
			\subsection{Proper Orthogonal Decomposition} % Method of Snapshots
			...
			\begin{equation}
				\dot{\mathbf{q}} = \nu L(\mathbf{q}) + Q(\mathbf{q},\mathbf{q}), \label{eq:qdot}
			\end{equation}
			where 
			\begin{align}
				L(\mathbf{q}) &= \begin{pmatrix}
					u_{xx} + u_{yy} \\ v_{xx} + v_{yy} \\ 0
				\end{pmatrix} \label{eq:L}\\
				Q(\mathbf{q}^1,\mathbf{q}^2) &= - \begin{pmatrix}
			...
			\begin{align*}
				\begin{split}
					b_k^1 &= \sprod{L(\bar{\mathbf{q}})}{\phi_k} \\
					L_{ik}^1 &= \sprod{L(\phi_i)}{\phi_k} \\
					Q_{ijk} &=\sprod{Q(\phi_i,\phi_j))}{\phi_k}.
				\end{split}
				\begin{split}
					b_k^2 &= \sprod{Q(\bar{\mathbf{q}},\bar{\mathbf{q}})}{\phi_k} \\
					L_{ik}^2 &=\sprod{Q(\bar{\mathbf{q}},\phi_i) + Q(\phi_i,\bar{\mathbf{q}}))}{\phi_k} \\
					\phantom{empty}					
				\end{split}
			\end{align*}
			...
			\begin{equation*}
				\nu L(\bar{\mathbf{q}}) + Q(\bar{\mathbf{q}},\bar{\mathbf{q}}) = 0
			\end{equation*} 
